resource "aws_lambda_function" "log_parser" {
  ...
  environment {
    variables = {
      ...
      IP_SET_ID_AUTO_BLOCK                           = "${aws_wafregional_ipset.waf_scanner_probe_set.id}"
...
resource "aws_lambda_function" "reputation_lists_parser" {
  function_name = "${var.waf_prefix}_${lower(var.stack_name)}_reputation_lists_parser"
...
resource "aws_lambda_function" "bad_bot_parser" {
  ...
  environment {
    variables = {
      IP_SET_ID_BAD_BOT         = "${aws_wafregional_ipset.waf_bad_bot_set.id}"
      SEND_ANONYMOUS_USAGE_DATA = "${var.send_anonymous_usage_data}"
      UUID                      = "${random_uuid.uuid.result}"
      REGION                    = "${var.aws_region}"
      LOG_TYPE                  = "${var.endpoint}"
...
resource "aws_lambda_function" "custom_resource" {
  function_name = "${var.waf_prefix}_${lower(var.stack_name)}_custom_resource"
